...
class Model:
    ...
    def _forward_propagation(self, x, thetas):
        ...
        current_layer = np.array([1.0,x])
        #current_layer = np.hstack((np.ones((rows,1)), x))

        theta_matrix1, theta_matrix2 = thetas
        hidden_layer = self._logistic(np.matmul(current_layer, theta_matrix1))
        layers.append(hidden_layer)
        output = np.matmul(hidden_layer, theta_matrix2)
        layers.append(output)
        
        # for index, theta_matrix in enumerate(thetas):
        #     current_layer = self._logistic(np.matmul(current_layer, theta_matrix))
        #     # if (index+1) != len(thetas)-1:
        #     #     rows = current_layer.shape[0]
        #     #     current_layer = np.hstack((np.ones((rows,)), current_layer))
        #     layers.append(current_layer)
        return layers

    def _derivative_forward_propagation(self, x, thetas):
        rows   = 1
        #layers = self._forward_propagation(x, theta)
