...
# Check accuracy for different width of the network
L2_error = [] # for ||phi_t-phi_a||
Loss_error = [] # for Loss function

N_Init = 20 # Number of random initialisation of the parameter guess for fitting 
...
plt.subplot(2,1,1)
plt.semilogy(width, L2_error, 'bo--', linewidth=2, markersize=12)
...
plt.legend([r'$||\Psi_t-\Psi_a||_{L^2}$', r'Theoretical $O(q^{-1/2})$'])
plt.xlabel('q')
...
array_errors = np.stack((L2_array, Loss_array, width_array), axis=0).T 
