...
from WithFiniteDifference import Network
...
''' 
Create an instance of the Network class which solves a differential equation.
...
Sol = Solution(x)
dSol = (Solution(x+1e-12)-Sol)/1e-12

Net = Network(f = F, trial_solution = Trial, hidden_layers=architecture, max_iter=max_iter)
Net.fit(training_set)
u, du = Net.phi(x)

plt.plot(x, u, label="Neural Network's solution")
plt.plot(x, Sol, label="Exact Solution")
plt.plot(x, du, label="Neural Network's derivative")
plt.plot(x, dSol, label="Exact Solution's derivative")
plt.legend()
plt.show()
